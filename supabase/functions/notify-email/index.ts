...
//
// Envia e-mails transacionais (troca recebida/respondida, conta aprovada,
// plantão designado) via Resend.
//
// SEGURANÇA — DATABASE WEBHOOK (obrigatório):
// No dashboard do Supabase, o Database Webhook que chama esta função deve ser
// configurado com o header:
//   Authorization: Bearer <valor de WEBHOOK_SECRET>
// E o secret WEBHOOK_SECRET deve existir em Edge Functions → Secrets
// (ex.: openssl rand -hex 32). A função NEGA (401) qualquer chamada sem esse
// header correto e falha de forma segura (500) se WEBHOOK_SECRET não estiver
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f1f5f9;margin:0;padding:24px 16px;">
  <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding:24px 24px 20px;">
      <h2 style="margin:0 0 14px;color:#0f172a;font-size:17px;font-weight:600;">${escapeHtml(titulo)}</h2>
      ${corpo}
...
function tagSetor(nome: string) {
  return `<span style="background:#e0f2fe;color:#0369a1;padding:2px 8px;border-radius:20px;font-size:12px;font-weight:600;">${escapeHtml(nome)}</span>`
}
...
Deno.serve(async (req) => {
  ...
  // ── Verificação de origem (fail closed) ────────────────────────────────────
  if (!WEBHOOK_SECRET) {
    console.error('WEBHOOK_SECRET não configurado — recusando processamento.')
        ...
        await sendEmail(
          ...
          emailLayout(
            ...
            `<p style="color:#475569;line-height:1.7;margin:0 0 12px;">
              Olá, <strong style="color:#0f172a;">${escapeHtml(para.nome)}</strong>!<br>
              <strong>${escapeHtml(de?.nome ?? 'Um colega')}</strong> quer te transferir o seguinte plantão:
            </p>
            <div style="background:#f0fdfa;border:1px solid #99f6e4;border-radius:10px;padding:14px 16px;margin:0 0 14px;">
              <p style="margin:0;font-weight:600;color:#0f172a;font-size:15px;">📅 ${escapeHtml(formatarData(plantao?.data ?? ''))}</p>
              ${plantao?.setores?.nome ? `<p style="margin:6px 0 0;">${tagSetor(plantao.setores.nome)}</p>` : ''}
              ${plantao?.tipos_turno ? `<p style="margin:6px 0 0;color:#475569;font-size:13px;">⏰ ${escapeHtml(plantao.tipos_turno.hora_inicio?.slice(0,5) ?? '')} – ${escapeHtml(plantao.tipos_turno.hora_fim?.slice(0,5) ?? '')}</p>` : ''}
            </div>
            ${record.mensagem ? `<p style="background:#f8fafc;border-left:3px solid #cbd5e1;padding:10px 14px;border-radius:0 8px 8px 0;font-style:italic;color:#64748b;margin:0 0 14px;">"${escapeHtml(record.mensagem)}"</p>` : ''}
        ...
        await sendEmail(
          ...
          emailLayout(
            ...
            `<p style="color:#475569;line-height:1.7;margin:0;">
              Olá, <strong style="color:#0f172a;">${escapeHtml(de.nome)}</strong>!<br>
              <strong>${escapeHtml(para?.nome ?? 'O médico')}</strong>
              ${aceita
                ? '<span style="color:#16a34a;font-weight:600;">aceitou</span>'
                : '<span style="color:#dc2626;font-weight:600;">recusou</span>'}
              sua transferência do plantão de <strong>${escapeHtml(formatarData(plantao?.data ?? ''))}</strong>
              ${plantao?.setores?.nome ? `(${escapeHtml(plantao.setores.nome)})` : ''}.
        ...
        await sendEmail(
          ...
          emailLayout(
            ...
            `<p style="color:#475569;line-height:1.7;margin:0 0 14px;">
              Olá, <strong style="color:#0f172a;">${escapeHtml(record.nome)}</strong>!<br>
      ...
      if (prof?.email) {
        await sendEmail(
          ...
          emailLayout(
            ...
            `<p style="color:#475569;line-height:1.7;margin:0 0 14px;">
              Olá, <strong style="color:#0f172a;">${escapeHtml(prof.nome)}</strong>!<br>
            ...
            <div style="background:#f0fdfa;border:2px solid #0d9488;border-radius:10px;padding:16px 18px;">
              <p style="margin:0;font-weight:700;color:#0f172a;font-size:16px;">📅 ${escapeHtml(formatarData(plantao?.data ?? ''))}</p>
              ${plantao?.setores?.nome ? `<p style="margin:8px 0 0;">${tagSetor(plantao.setores.nome)}</p>` : ''}
              ${plantao?.tipos_turno ? `<p style="margin:8px 0 0;color:#475569;font-size:13px;">⏰ ${escapeHtml(plantao.tipos_turno.hora_inicio?.slice(0,5) ?? '')} – ${escapeHtml(plantao.tipos_turno.hora_fim?.slice(0,5) ?? '')}</p>` : ''}
            </div>
