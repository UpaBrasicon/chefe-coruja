-- ─────────────────────────────────────────────────────────────────────────────
-- Fix — terminologia.buscar: exibir LOINC em pt-BR quando disponível
--
-- A função passou a preferir componente_pt/nome_curto_pt (tradução oficial
-- da variante linguística do LOINC) na coluna `descricao` e no `extra`.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF p_tabela = 'loinc' THEN
    RETURN QUERY
      SELECT 'loinc'::text,
             t.codigo,
             -- prefere a tradução pt-BR (COMPONENT); senão COMPONENT en; senão nome longo
...
-- ===========================================================================
-- DOWN — aplicar a versão anterior (git history da migration 00004).
-- ===========================================================================
