-- ─────────────────────────────────────────────────────────────────────────────
-- Fix — terminologia.buscar sem SQL dinâmico
--
-- A versão anterior usava format()/EXECUTE com %I/%L, o que quebrou a busca
-- ("column capitulo does not exist") porque a montagem dinâmica misturava
-- colunas de tabelas diferentes. Esta versão usa um RETURN QUERY por tabela
-- (sem dynamic SQL): mais legível, mais seguro e com o mesmo contrato.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF p_tabela = 'loinc' THEN
    RETURN QUERY
      SELECT 'loinc'::text,
             t.codigo,
             -- prefere a tradução pt-BR; senão COMPONENT en; senão nome longo
             coalesce(NULLIF(t.componente_pt, ''), NULLIF(t.componente, ''), t.nome_longo),
             jsonb_build_object('componente', t.componente, 'componente_pt', t.componente_pt, 'propriedade', t.propriedade, 'unidade_exemplo', t.unidade_exemplo, 'nome_curto', t.nome_curto, 'nome_curto_pt', t.nome_curto_pt, 'classe', t.classe),
             ts_rank(t.busca, v_query)
...
-- ===========================================================================
-- DOWN — reverter para a versão anterior (se necessário):
--   (aplicar o corpo antigo com dynamic SQL via git history)
-- ===========================================================================
