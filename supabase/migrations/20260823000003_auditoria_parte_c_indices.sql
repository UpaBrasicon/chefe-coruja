...
-- Gerado a partir do banco real: 19 colunas tenant + 65 FKs sem índice (dedup
-- = 72 únicos) + 7 compostos para tabelas quentes. Sem CONCURRENTLY (migration
-- da CLI roda em transação; 121 MB → lock de ms).
-- ─────────────────────────────────────────────────────────────────────────────
...
create index if not exists idx_censo_ocupacao_org_data on public.censo_ocupacao (organizacao_id, criado_em desc);
...
-- ===========================================================================
-- DOWN (resumo — índices não são reversíveis com precisão sem o catálogo;
--   o padrão do projeto é manter. Se precisar reverter:
--   drop index if exists public.idx_<tabela>_<coluna>; para cada um criado)
-- ===========================================================================
